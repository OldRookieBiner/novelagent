...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_project_agent_prompts_project_id", "project_agent_prompts", ["project_id"]
    )
...
def downgrade() -> None:
    # Drop triggers
    op.execute(
        "DROP TRIGGER IF EXISTS update_project_agent_prompts_updated_at ON project_agent_prompts;"
    )
    op.execute(
    ...
    # Drop tables
    op.drop_index(
        "ix_project_agent_prompts_project_id", table_name="project_agent_prompts"
    )

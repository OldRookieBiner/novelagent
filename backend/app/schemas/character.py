...
class CharacterGenerateRequest(BaseModel):
    ...
    roles: Optional[List[str]] = Field(None, description="指定角色类型列表，如 ['主角', '反派']")
    additional_context: Optional[str] = Field(None, description="额外上下文信息")


class RelationGenerateRequest(BaseModel):
    """AI 生成关系规划请求 Schema"""
    character_ids: Optional[List[int]] = Field(None, description="指定人物 ID 列表，为空则生成所有人物间的关系")
...
class CharacterOptimizeRequest(BaseModel):
    """AI 优化单个人物请求 Schema"""
    fields: Optional[List[str]] = Field(None, description="需要优化的字段列表，为空则优化所有可空字段")
    additional_context: Optional[str] = Field(None, description="额外上下文信息，如特定的优化方向")
